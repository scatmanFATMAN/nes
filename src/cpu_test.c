...
void
cpu_test_init() {
    memset(&cpu_test, 0, sizeof(cpu_test));
}
...
cpu_test_free() {
    ...
    if (cpu_test.opcode != NULL) {
        free(cpu_test.opcode);
    }
    if (cpu_test.SP != NULL) {
        free(cpu_test.SP);
    }
    ...
    if (cpu_test.flags != NULL) {
        free(cpu_test.flags);
    }
    if (cpu_test.cycles != NULL) {
        free(cpu_test.cycles);
    }
}
...
cpu_test_load() {
    ...
    cpu_test.lines = 8991;
    cpu_test.PC = calloc(cpu_test.lines, sizeof(uint16_t));
    cpu_test.opcode = calloc(cpu_test.lines, sizeof(uint8_t));
    cpu_test.SP = calloc(cpu_test.lines, sizeof(uint8_t));
    cpu_test.A = calloc(cpu_test.lines, sizeof(uint8_t));
    cpu_test.X = calloc(cpu_test.lines, sizeof(uint8_t));
    cpu_test.Y = calloc(cpu_test.lines, sizeof(uint8_t));
    cpu_test.flags = calloc(cpu_test.lines, sizeof(uint8_t));
    cpu_test.cycles = calloc(cpu_test.lines, sizeof(int));

    for (i = 0; i < cpu_test.lines; i++) {
        //F7A6  EA        NOP                             A:11 X:FF Y:15 P:25 SP:FB PPU:238,232 CYC:26457
        ret = sscanf(text[i], "%04hX  %02hhX%*40cA:%02hhX X:%02hhX Y:%02hhX P:%02hhX SP:%02hhX%*13cCYC:%d",
    ...
    cpu_test.loaded = true;
    return true;
}

bool
cpu_test_check(uint16_t PC, uint8_t opcode, uint8_t A, uint8_t X, uint8_t Y, uint8_t SP, uint8_t flags, int cycles) {
    bool match;

    match = PC == cpu_test.PC[cpu_test.index] &&
            opcode == cpu_test.opcode[cpu_test.index] &&
            A == cpu_test.A[cpu_test.index] &&
            X == cpu_test.X[cpu_test.index] &&
            Y == cpu_test.Y[cpu_test.index] &&
            SP == cpu_test.SP[cpu_test.index] &&
            flags == cpu_test.flags[cpu_test.index] &&
            cycles == cpu_test.cycles[cpu_test.index];

    if (!match) {
        printf("Mismatch at line %u\n", cpu_test.index + 1);
        printf(" %04X  %02X    A: %02X  X:%02X  Y:%02X  SP: %02X  Cycles: %d  Flags: %02X\n",
            cpu_test.PC[cpu_test.index], cpu_test.opcode[cpu_test.index], cpu_test.A[cpu_test.index], cpu_test.X[cpu_test.index], cpu_test.Y[cpu_test.index], cpu_test.SP[cpu_test.index], cpu_test.cycles[cpu_test.index], cpu_test.flags[cpu_test.index]);
    }
